    ...
    <div class="container">
        ...
        <div class="box">
            ...
            <table>
                ...
                {f"{''.join(f'<tr><td>{i+1}. {c}</td><td>{formatar_nota_br(notas_i[c])}</td></tr>' for i, c in enumerate(criterios_avaliacao))}"}
            </table>
        ...
        <div class="box">
            ...
            <table>
                ...
                {f"{''.join(f'<tr><td>{i+1}. {c}</td><td>{formatar_nota_br(notas_ii[c])}</td></tr>' for i, c in enumerate(criterios_avaliacao))}"}
            </table>
...
<body>
    <div class="container">
        ...
        # Calcula a nota geral ponderada automaticamente
        nota_geral_ponderada = (nota_final_escrito * 0.6) + (nota_final_apresentacao * 0.4)
        st.number_input("NOTA GERAL", min_value=0.0, max_value=10.0, step=0.01, value=round(nota_geral_ponderada, 2), disabled=True)
...
<body>
    <div class="container">
        ...
        <div class="box">
            ...
            <table>
               <tr><th>Critério</th><th>Nota</th></tr>
                {f"{''.join(f'<tr><td>{i+1}. {c}</td><td>{formatar_nota_br(notas_final_i[c])}</td></tr>' for i, c in enumerate(criterios_final))}"}
            </table>
        ...
        <div class="box">
            ...
            <table>
               <tr><th>Critério</th><th>Nota</th></tr>
                {f"{''.join(f'<tr><td>{i+1}. {c}</td><td>{formatar_nota_br(notas_final_ii[c])}</td></tr>' for i, c in enumerate(criterios_final))}"}
            </table>
           <p><strong>Média ponderada: {formatar_nota_br(media_ponderada_final_ii)}</strong></p>
        </div>

        <div class="notas-container">
            <div class="nota-item-aprovacao">
